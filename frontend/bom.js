...
function drawProcurement(){
 ...
 ${d.bom_stale?'<div class="proc-warning">Documents changed. The saved BOM is outdated; regenerate it before new supplier searches or purchases.</div>':''}
...
function procMaterials(){
 ...
 return `<div class="proc-toolbar"><div><h3>${d.bom.version?'Saved AI Bill of Materials':'Generate your Bill of Materials'}</h3><p class="proc-muted">${d.bom.version?`Generated ${esc(d.bom.created_at)} · ${esc(d.bom.model)} · ${esc(d.bom.coverage||'')}`:`${estimate.documents} documents · ${estimate.pages} pages · ${estimate.pdf_mb} MB. Generation uses two AI passes. Opening this page uses no AI credits.`}</p></div><button class="primary" data-auto="generate" ${active?'disabled':''}>${d.bom.version?'Regenerate BOM':'Generate AI BOM'}</button></div>
 <p class="proc-muted">AI identifies, consolidates and checks materials. Uncertain values remain unknown. Confidence labels are judgments, not measured accuracy. Material details are read-only.</p>
 ...
 ${d.items.length?`<div class="proc-toolbar"><input id="procSearch" aria-label="Search BOM" placeholder="Find a material or specification…" value="${esc(procurement.search)}"><a href="${procUrl()}/rfq" target="_blank" rel="noopener">Download quotation request ↗</a></div><div id="procMaterialTable">${procBOMTable()}</div>`:`<div class="proc-empty"><span class="${active?'proc-spinner':''}"></span><h3>${active?'AI is preparing the BOM':'Your documents are ready for AI'}</h3><p>${active?'Progress and worker heartbeat are shown above. You can leave this page and return.':'Generate once, then reopen the saved result as often as needed. Previous local-extraction records remain preserved.'}</p></div>`}
...
function procBOMTable(){
 ...
 return `<div class="proc-table-wrap"><table class="proc-table"><thead><tr><th>Material</th><th>Requirement</th><th>Budget</th><th>Evidence</th><th></th></tr></thead><tbody>${items.slice(procurement.page*procurement.pageSize,(procurement.page+1)*procurement.pageSize).map(i=>`<tr><td><strong>${esc(i.name)}</strong><small>${esc(i.codes.join(' · '))} · ${esc(i.category)}</small><small>${esc(i.specification)}</small></td><td>${i.quantity==null?'Unknown quantity':esc(i.quantity)} ${esc(i.unit)}<small>${esc(i.location||'Location not specified')}</small></td><td>${i.budget==null?'Unknown':procAmount(i.budget,esc(i.currency))}</td><td>${procBadge(i.status,i.conflicts.length?'warn':'good')}<small>${i.sources.length} citations · identity: ${esc(i.confidence_fields?.identity||'unknown')}</small></td><td><button data-auto="evidence" data-id="${i.id}">View evidence</button></td></tr>`).join('')||'<tr><td colspan="5">No matching materials.</td></tr>'}</tbody></table></div>${procPagination('materials',procurement.page,items.length)}`;
}
